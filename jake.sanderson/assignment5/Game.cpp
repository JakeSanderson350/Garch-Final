...
bool Game::init(unsigned int width, unsigned int height)
{
	...
	pEventSystemInstance->addListener((EventType)TOGGLE_PAUSE_UNIT_ANIMATION, this);
	pEventSystemInstance->addListener((EventType)TOGGLE_UNIT_ANIMATION, this);
	pEventSystemInstance->addListener((EventType)CHANGE_UNIT_CURRENT_ANIMATION, this);
	pEventSystemInstance->addListener((EventType)CHANGE_UNIT_ANIMATION_SPEED, this);
	pEventSystemInstance->addListener((EventType)SPAWN_UNIT, this);
	pEventSystemInstance->addListener((EventType)REMOVE_UNIT, this);
...
void Game::handleEvent(const Event& currentEvent)
{
	...
	else if (currentEventType == REMOVE_UNIT)
	{
		mpUnitManager->destroyUnit(mpInputSystem->getMousePos());
	}

	else if (currentEventType == CHANGE_UNIT_ANIMATION_SPEED && mpUnitManager->getSize() > 0)
	{
		const ChangeUnitAnimationSpeedEvent& animationSpeedEvent = static_cast<const ChangeUnitAnimationSpeedEvent&>(currentEvent);

		mpUnitManager->getLastUnit()->setAnimationSpeed(animationSpeedEvent.getAnimationSpeed());
	}

	else if (currentEventType == CHANGE_UNIT_CURRENT_ANIMATION && mpUnitManager->getSize() > 0)
	{
		...
		mpUnitManager->getLastUnit()->setCurrentAnimation(currentAnimationEvent.getCurrentAnimationIndex());
	}

	else if (currentEventType == TOGGLE_UNIT_ANIMATION && mpUnitManager->getSize() > 0)
	{
		mpUnitManager->toggleCurrentAnimation();
	}

	else if (currentEventType == TOGGLE_PAUSE_UNIT_ANIMATION)
	{
